package basico;

public class Variaveis {

    private String nomeSegur;
    private String cpfSegur;
    private String cep1;
    private String cep2;
    private String dataSegur;
    private String estadoCivil;
    private String sexoCondu;
    private String tipoSeguro;
    private String classeBonus;
    private String franquia;
    private String anoModelo;
    private String zeroKm;
    private String veiculo;
    private String qtdPortas;
    private String ciaAnterior;
    private String kmTrab;
    private String alienado;
    private String tipoRes;
    private String grgTrab;
    private String grgEscola;
    private String dirige25;
    private String sexo25;
    private String fatAjust;
    private String dm;
    private String dc;
    private String comissao;
    private String app;
    private String dmo;
    private String CarroRes;
    private String ass24;
    private String vidro;
    private String cpfcond;
    private String nomeCond;
    private String datacond;
    private String grgRes;
    private String portAut;
    private String tipoPessoa;
    private String relacaoseg;
    private String comercial;
    private String nrlinha;
    private String aceita;


    public String getAceita() {
        return aceita;
    }

    public void setAceita(String aceita) {
        this.aceita = aceita;
    }

    public String getNrlinha() {
        return nrlinha;
    }


    public void setNrlinha(String nrlinha) {
        this.nrlinha = nrlinha;
    }

    public String getNomeSegur() {
        return nomeSegur;
    }

    public void setNomeSegur(String nomeSegur) {
        this.nomeSegur = nomeSegur;
    }

    public String getCpfSegur() {
        return cpfSegur;
    }

    public void setCpfSegur(String cpfSegur) {
        this.cpfSegur = cpfSegur;
    }

    public String getCep1() {
        return cep1;
    }

    public void setCep1(String cep1) {
        this.cep1 = cep1;
    }

    public String getCep2() {
        return cep2;
    }

    public void setCep2(String cep2) {
        this.cep2 = cep2;
    }

    public String getDataSegur() {
        return dataSegur;
    }

    public void setDataSegur(String dataSegur) {
        this.dataSegur = dataSegur;
    }

    public String getEstadoCivil() {
        return estadoCivil;
    }

    public void setEstadoCivil(String estadoCivil) {
        this.estadoCivil = estadoCivil;
    }

    public String getSexoCondu() {
        return sexoCondu;
    }

    public void setSexoCondu(String sexoCondu) {
        this.sexoCondu = sexoCondu;
    }

    public String getTipoSeguro() {
        return tipoSeguro;
    }

    public void setTipoSeguro(String tipoSeguro) {
        this.tipoSeguro = tipoSeguro;
    }

    public String getClasseBonus() {
        return classeBonus;
    }

    public void setClasseBonus(String classeBonus) {
        this.classeBonus = classeBonus;
    }

    public String getFranquia() {
        return franquia;
    }

    public void setFranquia(String franquia) {
        this.franquia = franquia;
    }

    public String getAnoModelo() {
        return anoModelo;
    }

    public void setAnoModelo(String anoModelo) {
        this.anoModelo = anoModelo;
    }

    public String getZeroKm() {
        return zeroKm;
    }

    public void setZeroKm(String zeroKm) {
        this.zeroKm = zeroKm;
    }

    public String getVeiculo() {
        return veiculo;
    }

    public void setVeiculo(String veiculo) {
        this.veiculo = veiculo;
    }

    public String getQtdPortas() {
        return qtdPortas;
    }

    public void setQtdPortas(String qtdPortas) {
        this.qtdPortas = qtdPortas;
    }

    public String getCiaAnterior() {
        return ciaAnterior;
    }

    public void setCiaAnterior(String ciaAnterior) {
        this.ciaAnterior = ciaAnterior;
    }

    public String getKmTrab() {
        return kmTrab;
    }

    public void setKmTrab(String kmTrab) {
        this.kmTrab = kmTrab;
    }

    public String getAlienado() {
        return alienado;
    }

    public void setAlienado(String alienado) {
        this.alienado = alienado;
    }

    public String getTipoRes() {
        return tipoRes;
    }

    public void setTipoRes(String tipoRes) {
        this.tipoRes = tipoRes;
    }

    public String getGrgTrab() {
        return grgTrab;
    }

    public void setGrgTrab(String grgTrab) {
        this.grgTrab = grgTrab;
    }

    public String getGrgEscola() {
        return grgEscola;
    }

    public void setGrgEscola(String grgEscola) {
        this.grgEscola = grgEscola;
    }

    public String getDirige25() {
        return dirige25;
    }

    public void setDirige25(String dirige25) {
        this.dirige25 = dirige25;
    }

    public String getSexo25() {
        return sexo25;
    }

    public void setSexo25(String sexo25) {
        this.sexo25 = sexo25;
    }

    public String getFatAjust() {
        return fatAjust;
    }

    public void setFatAjust(String fatAjust) {
        this.fatAjust = fatAjust;
    }

    public String getDm() {
        return dm;
    }

    public void setDm(String dm) {
        this.dm = dm;
    }

    public String getDc() {
        return dc;
    }

    public void setDc(String dc) {
        this.dc = dc;
    }

    public String getComissao() {
        return comissao;
    }

    public void setComissao(String comissao) {
        this.comissao = comissao;
    }

    public String getApp() {
        return app;
    }

    public void setApp(String app) {
        this.app = app;
    }

    public String getDmo() {
        return dmo;
    }

    public void setDmo(String dmo) {
        this.dmo = dmo;
    }

    public String getCarroRes() {
        return CarroRes;
    }

    public void setCarroRes(String carroRes) {
        CarroRes = carroRes;
    }

    public String getAss24() {
        return ass24;
    }

    public void setAss24(String ass24) {
        this.ass24 = ass24;
    }

    public String getVidro() {
        return vidro;
    }

    public void setVidro(String vidro) {
        this.vidro = vidro;
    }

    public String getCpfcond() {
        return cpfcond;
    }

    public void setCpfcond(String cpfcond) {
        this.cpfcond = cpfcond;
    }

    public String getNomeCond() {
        return nomeCond;
    }

    public void setNomeCond(String nomeCond) {
        this.nomeCond = nomeCond;
    }

    public String getDatacond() {
        return datacond;
    }

    public void setDatacond(String datacond) {
        this.datacond = datacond;
    }

    public String getGrgRes() {
        return grgRes;
    }

    public void setGrgRes(String grgRes) {
        this.grgRes = grgRes;
    }

    public String getPortAut() {
        return portAut;
    }

    public void setPortAut(String portAut) {
        this.portAut = portAut;
    }

    public String getTipoPessoa() {
        return tipoPessoa;
    }

    public void setTipoPessoa(String tipoPessoa) {
        this.tipoPessoa = tipoPessoa;
    }

    public String getRelacaoseg() {
        return relacaoseg;
    }

    public void setRelacaoseg(String relacaoseg) {
        this.relacaoseg = relacaoseg;
    }

    public String getComercial() {
        return comercial;
    }

    public void setComercial(String comercial) {
        this.comercial = comercial;
    }


}
